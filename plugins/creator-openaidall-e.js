...
let handler = async (m, { conn, text, usedPrefix, command }) => {
  ...
  try {
    ...
    await conn.sendMsg(
      m.chat,
      { image: anu.data.getRandom(), caption: `Open AI Dall E :\n${text}` },
      { quoted: m }
    );
  } catch (e) {
    console.log(e);
    try {
      let res = await fetch(
        `https://api.lolhuman.xyz/api/dall-e?apikey=${apilol}&text=${encodeURIComponent(
          text
        )}`
      );
      let anu = Buffer.from(await res.arrayBuffer());
      if (Buffer.byteLength(anu) < 22000)
        throw Error(`[!] Error : Buffer not found.`);
      await conn.sendMsg(
        m.chat,
        { image: anu, caption: `Open AI Dall E :\n${text}` },
        { quoted: m }
      );
    } catch (e) {
      console.log(e);
