...
let handler = async (m, { conn, usedPrefix, command, isOwner, text }) => {
  let groups;
  try {
    groups = Object.values(await conn.groupFetchAllParticipating()).map(
      (v) => v.id
    );
  } catch {
    return;
  ...
  let chats = Object.entries(gc)
    .filter((v) => v[1].isBanned)
    .map(([v]) => v);
  let users = Object.entries(pc)
    .filter(
      (v) => v[1].banned && !(v[0].startsWith('212') || v[0].startsWith('265'))
    )
    .map(([v]) => v);
  let z = 1,
    y = 1,
    ...
    sisa,
    timer;
  let txt = `*Total Banned User : ${
    Object.entries(pc).filter((user) => user[1].banned).length
  ...
  for (let x of chats) {
    ...
    if (!gc[x].permaBan && timer <= 0) {
      ...
      sisa = gc[x].permaBan
        ? 'Status : Permanent Banned Group'
        : isNaN(timer)
        ? 'Status : Soft Banned'
        : `Mute : ${timer.toTimeString()}`;
      txt += `\n*[${z}] ${name}*\n`;
      txt += '┗⊱ ' + sisa;
      z++;
    }
