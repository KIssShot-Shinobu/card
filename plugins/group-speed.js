...
let handler = async (m, { conn }) => {
  ...
  let chats = Object.entries(Connection.store.chats).filter(
  ...
  txt += `OS Uptime :\n*${runtime(os.uptime())}*\n\n`;
  txt += `💬 Status :\n- *${
    groupsIn.length < groups.length ? groups.length : groupsIn.length
  }* Group Chats\n`;
  txt += `- *${groups.length}* Groups Joined\n`;
  txt += `- *${
    groupsIn.length < groups.length ? 0 : groupsIn.length - groups.length
  }* Groups Left\n`;
