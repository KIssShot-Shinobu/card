...
let handler = async (m, { conn, command, text }) => {
  ...
  if (chat.expired == 0)
    return m.reply('[ ! ] Tidak ada durasi, bot join permanen.');
  ...
  await conn.reply(
    ...
    `${
      command.includes('min') || command.includes('kurang')
        ? 'Mengurangi'
        : 'Menambah'
    } durasi join di grup sebanyak *${durasi} hari*`,
    m
  );
};
...
handler.command =
  /^(((t|n)ambah|add|plus|min|kurang)((gro?up|join|sewa)dura(si|tion)|dura(si|tion)(gro?up|join|sewa)))$/i;
