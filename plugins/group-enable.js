...
let handler = async (
  m,
  { conn, usedPrefix, command, args, isOwner, isBotAdmin, isAdmin, isROwner }
) => {
  ...
  switch (type) {
    ...
    case 'pconly':
    case 'privateonly':
      if (isEnable && global.opts['gconly'])
        throw `[!] Matikan dulu *gconly !*`;
      isAll = true;
      if (!isROwner) {
        global.dfail('rowner', m, conn);
        throw false;
    ...
    case 'gconly':
    case 'grouponly':
      if (isEnable && global.opts['pconly'])
        throw `[!] Matikan dulu *pconly !*`;
      isAll = true;
      if (!isROwner) {
        global.dfail('rowner', m, conn);
        throw false;
    ...
    case 'psgc':
      if ((isAdmin || isOwner) && isBotAdmin) {
        ...
        m.reply(
          isBotAdmin ? `*「ADMIN GROUP ONLY」*` : `*「BOT HARUS JADI ADMIN」*`
        );
      }
      break;
    default:
