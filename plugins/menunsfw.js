...
let handler = async (m, { conn, usedPrefix: _p, __dirname }) => {
  try {
    ...
    let nais =
      m.isGroup && !chat.nsfw
        ? 'https://i.ibb.co/0QfCNxj/nsfwoff.jpg'
        : await got(
            'https://raw.githubusercontent.com/clicknetcafe/Databasee/main/azamibot/nsfw.json'
          )
            .json()
            .then((v) => v.getRandom());
    let _package =
    ...
    let menunsfw = Object.values(plugins)
      .filter((plugin) => !plugin.disabled)
      .map((plugin) => {
        return {
          menunsfw: Array.isArray(plugin.tagsnsfw)
            ? plugin.menunsfw
            : [plugin.menunsfw],
          tagsnsfw: Array.isArray(plugin.tagsnsfw)
            ? plugin.tagsnsfw
            : [plugin.tagsnsfw],
    ...
    let _text = [
      ...
      ...Object.keys(tagsnsfw).map((tag) => {
        return (
          header.replace(/%category/g, tagsnsfw[tag]) +
          '\n' +
          [
            ...menunsfw
              .filter(
                (nsfwmenu) =>
                  nsfwmenu.tagsnsfw &&
                  nsfwmenu.tagsnsfw.includes(tag) &&
                  nsfwmenu.menunsfw
              )
              .map((nsfwmenu) => {
                return nsfwmenu.menunsfw
                  .map((menunsfw) => {
                    return body
                      .replace(
                        /%cmd/g,
                        nsfwmenu.prefix ? menunsfw : '%p' + menunsfw
                      )
                      .trim();
                  })
                  .join('\n');
    ...
    let text =
      typeof conn.nsfwmenu == 'string'
        ? conn.nsfwmenu
        : typeof conn.nsfwmenu == 'object'
        ? _text
        : '';
    let replace = {
      p: _p,
    ...
    await conn.sendFile(
      ...
      nais,
      '',
      text.replace(`<nuke_code>`, `<nuke_code>${readMore}`).trim(),
      m
    );
  } catch (e) {
    throw e;
