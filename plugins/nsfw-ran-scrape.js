...
let handler = async (m, { conn, usedPrefix, command }) => {
  ...
  try {
    let res = await fetch(
      `https://raw.githubusercontent.com/clicknetcafe/Databasee/main/nsfw/${command
        .replace('manga2nsfw', 'manga')
        .replace('nekonimensfw', 'nekonime')}.json`
    );
...
handler.command =
  /^(eba|foot|gifs|hentaivid|jahy|manga2nsfw|neko2|nekonimensfw|nsfwloli|nsfwmanga|nsfwneko|panties|zettai)$/i;
