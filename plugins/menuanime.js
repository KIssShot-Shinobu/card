...
let handler = async (m, { conn, usedPrefix: _p, __dirname }) => {
  try {
    ...
    let menuanime = Object.values(plugins)
      .filter((plugin) => !plugin.disabled)
      .map((plugin) => {
        return {
          menuanime: Array.isArray(plugin.tagsanime)
            ? plugin.menuanime
            : [plugin.menuanime],
          tagsanime: Array.isArray(plugin.tagsanime)
            ? plugin.tagsanime
            : [plugin.tagsanime],
    ...
    let _text = [
      ...
      ...Object.keys(tagsanime).map((tag) => {
        return (
          header.replace(/%category/g, tagsanime[tag]) +
          '\n' +
          [
            ...menuanime
              .filter(
                (animemenu) =>
                  animemenu.tagsanime &&
                  animemenu.tagsanime.includes(tag) &&
                  animemenu.menuanime
              )
              .map((animemenu) => {
                return animemenu.menuanime
                  .map((menuanime) => {
                    return body
                      .replace(
                        /%cmd/g,
                        animemenu.prefix ? menuanime : '%p' + menuanime
                      )
                      .trim();
                  })
                  .join('\n');
    ...
    let text =
      typeof conn.animemenu == 'string'
        ? conn.animemenu
        : typeof conn.animemenu == 'object'
        ? _text
        : '';
    let replace = {
      p: _p,
    ...
    await conn.sendFile(
      ...
      nais,
      '',
      text.replace(`PIC* 」`, `PIC* 」${readMore}`).trim(),
      m
    );
  } catch (e) {
    throw e;
