...
let handler = async (m, { conn, usedPrefix: _p, __dirname }) => {
  try {
    ...
    let menugroup = Object.values(plugins)
      .filter((plugin) => !plugin.disabled)
      .map((plugin) => {
        return {
          menugroup: Array.isArray(plugin.tagsgroup)
            ? plugin.menugroup
            : [plugin.menugroup],
          tagsgroup: Array.isArray(plugin.tagsgroup)
            ? plugin.tagsgroup
            : [plugin.tagsgroup],
    ...
    let _text = [
      ...
      ...Object.keys(tagsgroup).map((tag) => {
        return (
          header.replace(/%category/g, tagsgroup[tag]) +
          '\n' +
          [
            ...menugroup
              .filter(
                (groupmenu) =>
                  groupmenu.tagsgroup &&
                  groupmenu.tagsgroup.includes(tag) &&
                  groupmenu.menugroup
              )
              .map((groupmenu) => {
                return groupmenu.menugroup
                  .map((menugroup) => {
                    return body
                      .replace(
                        /%cmd/g,
                        groupmenu.prefix ? menugroup : '%p' + menugroup
                      )
                      .trim();
                  })
                  .join('\n');
    ...
    let text =
      typeof conn.groupmenu == 'string'
        ? conn.groupmenu
        : typeof conn.groupmenu == 'object'
        ? _text
        : '';
    let replace = {
      p: _p,
    ...
    await conn.sendFile(
      ...
      nais,
      '',
      text.replace(`demote @tag`, `demote @tag${readMore}`).trim(),
      m
    );
  } catch (e) {
    throw e;
