...
const handler = async (m, { conn, text }) => {
  ...
  try {
    if(query === undefined || query === '') {
         await conn.sendMessage(m.chat, { text: 'Gunakan perintah dengan benar\n.art [nama kartu]' }, { quoted: m });
    }else {
       const endpoint = `https://db.ygoprodeck.com/api/v7/cardinfo.php?name=${encodeURIComponent(query)}`;
    ...
    if (data?.data?.length > 0) {
      ...
      if (cardInfo.card_images?.length > 0) {
        ...
        const cardImageUrl = cardInfo.card_images[randomIndex].image_url_cropped;
    ...
    console.log(error);
    await conn.sendMessage(m.chat, { text: 'Nama kartu yang anda masukan salah.' }, { quoted: m });
  }
  }
};
