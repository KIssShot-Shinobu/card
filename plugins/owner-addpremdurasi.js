...
let handler = async (m, { conn, command, args }) => {
  ...
  if (!prems.map((v) => v.user).includes(who))
    return m.reply(`[ ! ] User tidak ada dalam list premium.`);
  if ((m.quoted && !args[0]) || (!m.quoted && !args[1] && m.isGroup))
    return m.reply(`[!] Masukkan durasi *( dalam hari )*`);
  if (
    (m.quoted && isNaN(args[0])) ||
    (!m.quoted && isNaN(args[1]) && m.isGroup)
  )
    return m.reply(`[!] Durasi harus dalam format angka.`);
  ...
  await conn.reply(
    m.chat,
    `${
      command.includes('min') || command.includes('kurang')
        ? 'Mengurangi'
        : 'Menambah'
    } durasi premium @${(who || '').replace(
      /@s\.whatsapp\.net/g,
      ''
    )} sebanyak *${durasi} hari*`,
    m,
    { mentions: [who] }
  );
};
...
handler.command =
  /^(((t|n)ambah|add|plus|min|kurang)(prem(ium)?dura(si|tion)|dura(si|tion)prem(ium)?))$/i;
