...
let handler = async (
  m,
  { conn, text, participants, groupMetadata, isBotAdmin }
) => {
  let chatt = text.endsWith('@g.us') ? text : m.chat;
  try {
    let pp =
      (await conn.profilePictureUrl(chatt, 'image').catch((_) => null)) ||
...
*isBotAdmin :* ${
      x.participants.filter((v) => v.id == conn.user.jid && v.admin == 'admin')
        .length == 0
        ? 'No'
        : 'Yes'
    }
*Ephemeral :* ${
      x.ephemeralDuration ? `${x.ephemeralDuration / 86400} Day(s)` : 'Off'
    }
*Edit Group Info :* ${x.restrict ? 'Only Admins' : 'All Participants'}
*Send Messages :* ${x.announce ? 'Only Admins' : 'All Participants'}
_last group name changed by ${
      x.subjectOwner ? '@' + x.subjectOwner.split('@')[0] : 'Unknown'
    } on ${new Date(x.subjectTime * 1000).toDateString()}_
...
${
  x.desc
    ? x.desc
        .toString()
        .replace(
          /chat.whatsapp.com\/(?:invite\/)?([0-9A-Za-z]{20,24})/g,
          '#HIDDENbyDefault'
        )
    : 'None'
}
...
*Demote :* ${sDemote}`;
    ...
    await conn.sendFile(
      ...
      `${
        isBotAdmin
          ? txt
          : `${txt.replaceAll('chat.whatsapp.com', 'chat.whatsapp|com')}`
      }`,
      m,
      false,
      { mentions: y }
