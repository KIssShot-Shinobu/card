...
let handler = async (m, { conn, usedPrefix, command, text }) => {
  ...
  if (/image/g.test(mime) && !/webp/g.test(mime)) {
    ...
    try {
      let anu = await AI2D(out, {
        crop: `${/twin|duo|dual|compare/g.test(text) ? 'COMPARED' : 'SINGLE'}`,
      });
