...
let handler = async (
  m,
  { conn, participants, usedPrefix, command, args, isOwner, isPrems, isAdmin }
) => {
  let admins = [];
  ...
  let data = [
    ...ow.rowner.filter(([id, isCreator]) => id && isCreator),
    ...ow.owner.filter(([id, isCreator]) => id && isCreator),
  ].map((v) => v[0] + '@s.whatsapp.net');
  if (somematch(data, who) || who == conn.user.jid) throw `Gaboleh gitu :v`;
  if (isOwner || isAdmin || isPrems) {
    if (somematch(admins, who) && !isOwner)
      throw `Gaboleh gitu sesama admin :v`;
    if (total > 200 && !isPrems)
      throw `_... >> not premium ..._\n[!] Maksimal ${command} : 200 menit.`;
    ...
    if (users.permaban)
      throw `[!] Tidak perlu *${command}* karena sudah di *ban*`;
    ...
    await conn.reply(
      who,
      `User @${who.split('@')[0]} di *mute* selama ${total} menit.`,
      fliveLoc,
      { mentions: [who] }
    );
  } else {
