...
let handler = async (m, { conn, args, usedPrefix, command }) => {
  if (!args[1] || !somematch(one, args[0]))
    throw `*Format :*\n${
      usedPrefix + command
    } [opts] all / tag / quote\n\n*Example :*\n${
      usedPrefix + command
  ...
  let who,
    meh,
    enable = somematch(one.slice(0, 4), args[0]);
  ...
  let ya = who
    ...
    : 'Semua User';
  let chat = `Forward Status ${ya} telah ${
  ...
  if (enable)
    await conn.reply(m.sender, sender, fkontak, men ? { mentions: [men] } : {});
