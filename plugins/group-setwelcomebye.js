...
let handler = async (m, { conn, command, usedPrefix, text }) => {
    ...
    return await conn.reply(
      ...
      `[ ! ] Masukkan teks.\n\n*Tips :*\n@user - menampilkan tag user.${
        ...
        command.includes('bye')
          ...
          : `*Contoh :*\n${
              usedPrefix + command
            } Hallo @user, Selamat Datang di @subject\n\n*Hasil teks :*\nHallo @${
              m.sender.split('@')[0]
            }, Selamat Datang di ${subject}`
      }`,
      m,
  ...
  if (text.toLowerCase() == 'default' || text.toLowerCase() == 'reset')
    text = command.includes('bye') ? bye : wel;
  if (command.includes('bye')) chat.sBye = text;
  else chat.sWelcome = text;
  await m.reply(
    `Berhasil mengubah *${
      command.includes('bye') ? 'Bye' : 'Welcome'
    }* menjadi :\n\n${text}`
  );
};
