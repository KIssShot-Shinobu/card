...
let handler = async (m, { conn }) => {
  ...
  if (!canLevelUp(user.level, user.exp, global.multiplier)) {
    ...
    try {
      ...
      let out = await got(
        'https://raw.githubusercontent.com/clicknetcafe/Databasee/main/azamibot/menus.json'
      )
        .json()
        .then((v) => v.getRandom());
      image = await new can.Rank()
        .setAvatar(pp)
        .setUsername(name.replaceAll('\n', ''))
        .setBg(out)
        .setNeedxp(xp)
    ...
    if (before !== user.level) {
      let txt = `Selamat ${name.replaceAll(
        ...
        user.level
      }\n• Pada Jam : ${new Date().toLocaleString(
        'id-ID'
